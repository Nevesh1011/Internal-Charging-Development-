#!/usr/bin/env python3
"""
Customer Bucket Expiry Utility
-------
Standalone utility to validate and update CUSTOMER_BUCKET expiry status.
...
# ---------------------------------------------------------------------
# FIND ACTIVE BUCKETS WHOSE END TIME HAS EXPIRED
# ---------------------------------------------------------------------
def get_expiry_candidates(conn):
    """
    Return only buckets that require expiry update:
      1. current status is ACTIVE or legacy active value 1
      2. valid_end_time is populated
      3. valid_end_time <= database CURRENT_TIMESTAMP
    """
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            WHERE UPPER(TRIM(status::text)) IN ('ACTIVE', '1')
...
# VALIDATE STATUS
# ---------------------------------------------------------------------
def validate_expiry_buckets(rows):
    """
    The SELECT query already returns only rows that must be expired:
      - status is ACTIVE or legacy active value 1
      - valid_end_time IS NOT NULL
      - valid_end_time <= CURRENT_TIMESTAMP
    """
    return rows
...
# ---------------------------------------------------------------------
def print_expired_buckets(rows):
    if not rows:
        print("No ACTIVE buckets found with expired valid_end_time.")
        return

    print()
    print("=" * 100)
    print("CUSTOMER BUCKETS TO BE EXPIRED")
    print("=" * 100)
    ...
    print("=" * 100)
    print(f"Total number of buckets to update: {len(rows)}")
...
def run_check():
    with get_connection() as conn:
        ...
        expired_rows = validate_expiry_buckets(rows)

        print_expired_buckets(expired_rows)

        logger.info(
            "CHECK COMPLETE | expired_buckets=%s",
            len(expired_rows)
        )
...
def run_update():
    with get_connection() as conn:
        ...
        expired_rows = get_expiry_candidates(conn)
...
def main():
    ...
    mode.add_argument(
        "--check",
        action="store_true",
        help="Show only ACTIVE buckets whose valid_end_time has expired. No DB update."
    )

    mode.add_argument(
        "--update",
        action="store_true",
        help="Update ACTIVE buckets with expired valid_end_time to EXPIRED."
    )
