...
REQUIRED_TABLES=['bucket_request_inbound','bucket_request','external_id_ref','pop_usage_spec_group','usg_bucket_grp','po_bucket_grp','bucket_definition','bucket_validity_period','validity_unit','bucket_triggers','customer_bucket']
REQUIRED_COLUMNS={
...
'customer_bucket':['bucket_id','customer_ban','status','bucket_usage_type','bucket_validity_period','bucket_usage_group','bucket_trigger_type','order_id','product_offering_id','pop_id','product_spec_char_value_use_id','timeband_start','timeband_end','valid_start_time','valid_end_time','created_at','last_updated_at','bucket_unit','bucket_initial_value','bucket_remaining_value']}
...
def insert_bucket(conn,r,g,b,v,po_map):
    ...
    with conn.cursor() as cur:
        cur.execute(f"""INSERT INTO {DB_SCHEMA}.customer_bucket(bucket_id,customer_ban,status,bucket_usage_type,bucket_validity_period,bucket_usage_group,bucket_trigger_type,order_id,product_offering_id,pop_id,product_spec_char_value_use_id,timeband_start,timeband_end,valid_start_time,valid_end_time,created_at,last_updated_at,bucket_unit,bucket_initial_value,bucket_remaining_value) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,%s,%s,%s)""",(bid,r['customer_ban'],CUSTOMER_BUCKET_ACTIVE_STATUS,b['bucket_usage_type'],v['validity_period_id'],str(g),r['bucket_trigger_type'],r['order_id'],r['product_offering_id'],str(r['pop_id']),po_map['product_characteristic_value_id'],v['timeband_start'],v['timeband_end'],start,end,b['bucket_unit'],initial,initial))
...
def process_request(conn,r):
    ...
    for row in groups:
        ...
        bids.append(
            insert_bucket(conn,r,g,b,v,po_map)
        )
...
def main():
    print("Bucket Engine V2.1 started")

    logger.info('='*100)
    logger.info('Bucket Engine V2.1 starting')
    ...
    try:
        ...
        print("Error Count      : 1")
        print("")
        print("Bucket Engine V2.1 completed with errors")
    ...
    try:
        ...
        logger.info('Bucket Engine V2.1 completed')
        ...
        if inbound_received==0 and total==0:
            print("Bucket Engine V2.1 completed - No pending requests")
        elif overall_errors==0:
            print("Bucket Engine V2.1 completed successfully")
