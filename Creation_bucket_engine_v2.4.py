...
# Example: bucket_engine_v2_4_20260817_181530.log
RUN_TIMESTAMP=datetime.now().strftime('%Y%m%d_%H%M%S')
LOG_FILE=os.path.join(LOG_DIR,f'bucket_engine_v2_{RUN_TIMESTAMP}.log')

logger=logging.getLogger('bucket_engine_v2'); logger.setLevel(logging.INFO); logger.handlers.clear()
...
'pop_usage_spec_group':['pop_id','usage_spec_id','tenant_id'],
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            if mt or mc:
                details=[]
                if mt:
                    details.append('Missing tables: ' + ', '.join(f'{DB_SCHEMA}.{x}' for x in mt))
                if mc:
                    details.append('Missing columns: ' + ', '.join(f'{DB_SCHEMA}.{x}' for x in mc))
                raise BucketProcessingError(
                    'CONFIGURATION_ERROR',
                    'Database configuration validation failed. ' + ' | '.join(details)
                )
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                SELECT EXISTS(
                    ...
                    WHERE c.relkind = 'S'
                      AND n.nspname = %s
                      AND c.relname = 'customer_bucket_id_seq'
                ) AS exists
                """,
                (DB_SCHEMA,)
            )
            ...
            if not cur.fetchone()['exists']:
                raise BucketProcessingError(
                    'CONFIGURATION_ERROR',
                    f'Missing sequence: {DB_SCHEMA}.customer_bucket_id_seq'
                )

    logger.info(
        'STARTUP | SEQUENCE OK | %s.customer_bucket_id_seq',
        DB_SCHEMA
    )
...
def print_configuration_report():
    """Print database configuration details ONLY when an error is found."""
    missing_tables = []
    missing_columns = []
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            sequence_exists = cur.fetchone()['exists']

    if not sequence_exists:
        missing_tables.append('SEQUENCE customer_bucket_id_seq')

    # Keep console completely quiet when configuration is valid.
    ...
    for table in missing_tables:
        print(f"[MISSING TABLE]   {DB_SCHEMA}.{table}")

    for table_col in missing_columns:
        print(f"[MISSING COLUMN]  {DB_SCHEMA}.{table_col}")
...
def get_usage_spec(conn,r):
    ...
    with conn.cursor() as cur:
        cur.execute(
            f"""SELECT usage_spec_id,tenant_id
                FROM {DB_SCHEMA}.pop_usage_spec_group
                WHERE pop_id=%s
                ORDER BY usage_spec_id""",
            (r['pop_id'],)
        )
        rows=cur.fetchall()
    ...
    if not rows:
        raise BucketProcessingError(
            'USAGE_SPEC_NOT_FOUND',
            f"No usage spec configuration for POP_ID={r['pop_id']}"
        )
    if len(rows)>1:
        raise BucketProcessingError(
            'MULTIPLE_USAGE_SPEC_FOUND',
            f"Multiple usage spec configurations for POP_ID={r['pop_id']}"
        )

    u=rows[0]['usage_spec_id']
    tenant_id=rows[0]['tenant_id']

    if tenant_id is None:
        raise BucketProcessingError(
            'TENANT_CONFIGURATION_MISSING',
            f"Tenant ID is missing in POP_USAGE_SPEC_GROUP for POP_ID={r['pop_id']} USAGE_SPEC_ID={u}"
        )

    logger.info(
        'REQUEST | %s | STEP2 SUCCESS | POP=%s -> USAGE_SPEC=%s TENANT=%s',
        r['request_id'],r['pop_id'],u,tenant_id
    )
    return u,tenant_id

def get_groups(conn,r,u,tenant_id):
    logger.info(
        'REQUEST | %s | STEP3 USG_BUCKET_GRP usage_spec_id=%s tenant=%s',
        r['request_id'],u,tenant_id
    )
    with conn.cursor() as cur:
        cur.execute(
            f"""SELECT bucket_type_group_id,tenant_id
                FROM {DB_SCHEMA}.usg_bucket_grp
                WHERE usage_spec_id=%s
                  AND tenant_id=%s
    ...
    if not rows:
        raise BucketProcessingError(
            ...
            f'No bucket group configuration for USAGE_SPEC_ID={u} TENANT_ID={tenant_id}'
        )

    logger.info(
        'REQUEST | %s | STEP3 SUCCESS | groups=%s',
        r['request_id'],
        [(x['bucket_type_group_id'],x['tenant_id']) for x in rows]
    )
    return rows
...
def generate_bucket_id(conn):
    """
    Generate the next CUSTOMER_BUCKET bucket_id from PostgreSQL sequence.
    CUSTOMER_BUCKET.bucket_id remains VARCHAR; numeric sequence value is
    converted to string before insert.

    Expected sequence:
        customer_bucket_id_seq
    Expected start:
        1000
    """
    with conn.cursor() as cur:
        cur.execute(
            f"""
            SELECT nextval('{DB_SCHEMA}.customer_bucket_id_seq') AS bucket_id
            """
        )
        row = cur.fetchone()
    ...
    if not row or row['bucket_id'] is None:
        raise BucketProcessingError(
            ...
            'Unable to generate bucket ID from customer_bucket_id_seq'
        )
...
def insert_bucket(conn,r,g,b,v):
    ...
    bid=generate_bucket_id(conn); start=datetime.now(); end=calc_end(start,v['validity_duration'],v['validity_unit_name']); initial=b['bucket_initial_value']
    with conn.cursor() as cur:
    ...
    logger.info('REQUEST | %s | STEP9 SUCCESS | bucket_id=%s status=%s start=%s end=%s initial=%s remaining=%s',r['request_id'],bid,CUSTOMER_BUCKET_ACTIVE_STATUS,start,end,initial,initial); return bid
...
def process_request(conn,r):
    ...
    usage_spec_id, tenant_id = get_usage_spec(conn,r)

    # STEP 3: Usage Specification -> Bucket Group(s)
    groups = get_groups(conn,r,usage_spec_id,tenant_id)
...
def main():
    print("Bucket Engine V2.4 started")

    logger.info('='*100)
    logger.info('Bucket Engine V2.4 starting')
    ...
    try:
        ...
        logger.error('STARTUP FAILED | code=%s | message=%s',e.code,e.message)
        print("")
        print("STARTUP VALIDATION FAILED")
        print("Error Type    : CONFIGURATION ERROR")
        print(f"Error Code    : {e.code}")
        print(f"Details       : {e.message}")
        print(f"Log File      : {LOG_FILE}")
        print("")
        print("Bucket Engine V2.4 NOT started because database configuration is incomplete.")
        sys.exit(1)
    except Exception as e:
        logger.exception('STARTUP FAILED | unexpected error')
        print("")
        print("STARTUP VALIDATION FAILED")
        print("Error Type    : SYSTEM / DATABASE ERROR")
        print(f"Details       : {type(e).__name__}: {e}")
        print(f"Log File      : {LOG_FILE}")
        print("")
        print("Bucket Engine V2.4 NOT started.")
        sys.exit(1)
    ...
    try:
        ...
        logger.info('Bucket Engine V2.4 completed')
        ...
        if total == 0:
            print("Bucket Engine V2.4 completed - No pending requests")
        elif success == total:
            print("Bucket Engine V2.4 completed successfully")
        elif failed == total:
            print("Bucket Engine V2.4 completed with errors")
        else:
            print("Bucket Engine V2.0 partially completed")
