...
def find_processed_request_duplicate(conn,r):
    logger.info(
        'REQUEST | %s | PRECHECK duplicate request BAN=%s SUBSCR=%s ORDER=%s',
        r['request_id'],
        r['customer_ban'],
        r['subscr_id'],
        r['order_id']
    ...
    if existing:
        logger.warning(
            'REQUEST | %s | DUPLICATE REQUEST | existing_request_id=%s existing_bucket_id=%s',
            r['request_id'],
            existing['request_id'],
        ...
        logger.info(
            'REQUEST | %s | PRECHECK SUCCESS | no previous SUCCESS request with same BAN+SUBSCR+ORDER',
            r['request_id']
        )
...
def find_existing(conn,r,g):
    logger.info(
        'REQUEST | %s | STEP8 checking existing bucket by BAN + PRODUCT + BUCKET_GROUP',
        r['request_id']
    )
    ...
    if x:
        logger.warning(
            'REQUEST | %s | STEP8 DUPLICATE BUCKET | existing_bucket_id=%s existing_order_id=%s status=%s',
            r['request_id'],
            x['bucket_id'],
            x['order_id'],
            x['status']
        ...
        logger.info(
            'REQUEST | %s | STEP8 SUCCESS | no existing valid bucket',
            r['request_id']
        )
...
def process_request(conn,r):
    ...
    if duplicate_request:
        raise BucketProcessingError(
            ...
            (
                f"Request already processed successfully for "
                f"CUSTOMER_BAN={r['customer_ban']}, "
                f"SUBSCR_ID={r['subscr_id']}, "
