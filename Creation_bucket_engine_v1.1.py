...
LOG_FILE=os.path.join(LOG_DIR,'bucket_engine_v1.log')
...
def find_existing(conn,r,g):
    with conn.cursor() as cur:
        cur.execute(f"""SELECT bucket_id FROM {DB_SCHEMA}.customer_bucket WHERE customer_ban::text=%s AND product_offering_id::text=%s AND order_id::text=%s AND bucket_usage_group::text=%s LIMIT 1""",(r['customer_ban'],r['product_offering_id'],r['order_id'],str(g))); x=cur.fetchone()
    if x: logger.warning('REQUEST | %s | STEP8 DUPLICATE | bucket_id=%s',r['request_id'],x['bucket_id'])
    else: logger.info('REQUEST | %s | STEP8 SUCCESS | no duplicate',r['request_id'])
    return x
...
def insert_bucket(conn,r,g,b,v):
    ...
    logger.info('REQUEST | %s | STEP9 SUCCESS | bucket_id=%s start=%s end=%s initial=%s remaining=%s',r['request_id'],bid,start,end,initial,initial); return bid
...
def process_request(conn,r):
    ...
    for row in groups:
        g=row['bucket_group_id']; tenant_id=row['tenant_id']
    ...
    for r in reqs:
        ...
        with get_connection() as conn:
            try:
                ...
                conn.rollback(); mark_failed(conn,rid,e.code,e.message); conn.commit(); failed+=1
            except Exception as e:
                conn.rollback(); mark_failed(conn,rid,'UNEXPECTED_ERROR',f'{type(e).__name__}: {e}'); conn.commit(); logger.error('REQUEST | %s | TRACEBACK\n%s',rid,traceback.format_exc()); failed+=1
    return total,success,failed

def main():
    logger.info('='*100); logger.info('Bucket Engine V1 starting'); logger.info('Python=%s',sys.version.replace('\n',' ')); logger.info('Script path=%s | Log path=%s',SCRIPT_DIR,LOG_FILE); logger.info('DB host=%s port=%s db=%s user=%s schema=%s batch=%s',DB_HOST,DB_PORT,DB_NAME,DB_USER,DB_SCHEMA,BATCH_SIZE)
    try: validate_database_structure()
    except Exception: logger.exception('STARTUP FAILED'); sys.exit(1)
    total,success,failed=process_batch(); logger.info('RUN SUMMARY | total=%s | success=%s | failed=%s',total,success,failed); logger.info('Bucket Engine V1 completed')
