...
REQUIRED_TABLES=['bucket_request_inbound','bucket_request','external_id_ref','pop_usage_spec_group','usg_bucket_grp','pop_bucket_grp','po_bucket_grp','bucket_definition','bucket_validity_period','validity_unit','bucket_triggers','bucket_rollover_config','customer_bucket']
REQUIRED_COLUMNS={
                ...
                WHERE bri.engine_request_id IS NOT NULL
                  ...
                  AND (
                        ...
                        OR (br.process_status='SUCCESS' AND (
                              ...
                              OR bri.unit IS DISTINCT FROM cb.bucket_unit
...
def update_bucket_request_resolution(conn,r,usage_spec_id,g,po_map,b,rollover):
    """
    ...
    logger.info(
        'REQUEST | %s | AUDIT SNAPSHOT | usage_spec=%s group=%s char_id=%s type=%s size=%s unit=%s rollover=%s cap=%s cap_unit=%s',
        r['request_id'],usage_spec_id,g,po_map['product_characteristic_value_id'],
        b['bucket_usage_type'],b['bucket_initial_value'],b['bucket_unit'],
...
def process_request(conn,r):
    ...
    for row in groups:
        ...
        update_bucket_request_resolution(
            conn,r,usage_spec_id,g,po_map,b,rollover
        )
...
def main():
    print("Bucket Engine V3.8 started")

    logger.info('='*100)
    logger.info('Bucket Engine V3.8 starting')
    ...
    try:
        ...
        print("Error Count      : 1")
        print("")
        print("Bucket Engine V3.8 completed with errors")
    ...
    try:
        ...
        logger.info('Bucket Engine V3.8 completed')
        ...
        if inbound_received==0 and total==0:
            print("Bucket Engine V3.8 completed - No pending requests")
        elif overall_errors==0:
            print("Bucket Engine V3.8 completed successfully")
        elif (inbound_submitted>0 and success>0):
            print("Bucket Engine V3.8 partially completed")
        else:
            print("Bucket Engine V3.8 completed with errors")
