...
REQUIRED_TABLES=['bucket_request_inbound','bucket_request','bucket_request_error_history','external_id_ref','pop_usage_spec_group','usg_bucket_grp','pop_bucket_grp','po_bucket_grp','bucket_definition','bucket_validity_period','validity_unit','bucket_triggers','bucket_rollover_config','uom_ref','customer_bucket']
REQUIRED_COLUMNS={
...
'customer_bucket':['bucket_id','customer_ban','subscr_id','tenant_id','status','usage_spec_id','bucket_usage_type','bucket_validity_period','bucket_usage_group','bucket_trigger_type','bucket_trigger_id','order_id','product_offering_id','pop_id','product_spec_char_value_use_id','is_rollover_enabled','rollover_cap_value','rollover_cap_unit','recurrence_type','validity_year','validity_month','day_of_month','day_of_week_mask','timeband_start','timeband_end','next_day_end','priority','valid_start_time','valid_end_time','created_at','last_updated_at','bucket_unit','bucket_initial_value','bucket_remaining_value']}
...
def get_validity(conn,r,vid,g,tenant_id):
    ...
    """
    logger.info(
        'REQUEST | %s | STEP6 BUCKET_VALIDITY_PERIOD id=%s group=%s tenant=%s',
    ...
    if recurrence == 'MONTHLY':
        dom=v['day_of_month']
        if dom is None or int(dom) < 1 or int(dom) > 31:
            raise BucketProcessingError(
                'INVALID_MONTHLY_RECURRENCE',
                f'MONTHLY recurrence requires day_of_month 1-31 for validity_period_id={vid}'
            )
    ...
    logger.info(
        'REQUEST | %s | STEP6 SUCCESS | id=%s name=%s lifetime=%s %s '
...
def find_existing(conn,r,g,usage_spec_id):
    """
    Reject another currently usable bucket for the same subscriber entitlement.

    Runtime identity used here:
      BAN + SUBSCR_ID + PRODUCT_OFFERING_ID + USAGE_SPEC_ID + BUCKET_GROUP

    CLOSED/EXHAUSTED/expired buckets do not block a new creation.
    """
    logger.info(
        'REQUEST | %s | PRECHECK BUCKET checking BAN + SUBSCR + PRODUCT + USAGE_SPEC + BUCKET_GROUP',
        r['request_id']
    )
...
def insert_bucket(conn,r,usage_spec_id,g,tenant_id,b,v,po_map,trigger,rollover):
    """
    ...
    logger.info(
        'REQUEST | %s | STEP9 SUCCESS | bucket_id=%s status=%s usage_spec=%s group=%s recurrence=%s priority=%s valid=%s..%s initial=%s remaining=%s',
        r['request_id'],bid,CUSTOMER_BUCKET_ACTIVE_STATUS,usage_spec_id,g,
...
def process_request(conn,r):
    ...
    for row in groups:
        ...
        existing = find_existing(conn,r,g,usage_spec_id)
        ...
        bids.append(
            insert_bucket(conn,r,usage_spec_id,g,tenant_id,b,v,po_map,trigger,rollover)
        )
