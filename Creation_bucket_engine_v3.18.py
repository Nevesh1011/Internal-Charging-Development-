...
REQUIRED_TABLES=['bucket_request_inbound','bucket_request','bucket_request_error_history','external_id_ref','pop_usage_spec_group','usg_bucket_grp','pop_bucket_grp','po_bucket_grp','bucket_definition','bucket_validity_period','validity_unit','bucket_triggers','bucket_rollover_config','uom_ref','billing_cycle_config','billing_cycle_calendar','customer_bucket']
REQUIRED_COLUMNS={
'bucket_request_inbound':['inbound_request_id','order_id','customer_ban','subscr_id','status','bucket_usage_type','bucket_trigger_type','bucket_size','unit','product_offering_id','pop_id','product_characteristic_value_id','is_rollover_enabled','rollover_cap_value','rollover_cap_unit','timeband_start','timeband_end','activation_time','termination_time','bill_period','billing_frequency','request_status','engine_request_id','customer_bucket_id','error_code','error_message','received_dt','processed_dt','updated_dt'],
'bucket_request':['request_id','customer_ban','subscr_id','product_offering_id','pop_id','order_id','bucket_trigger_type','bucket_trigger_id','process_status','process_attempt_count','customer_bucket_id','error_code','error_message','processed_dt','updated_dt','usage_spec_id','bucket_type_group_id','product_characteristic_value_id','bucket_usage_type','bucket_size','bucket_unit','is_rollover_enabled','rollover_cap_value','rollover_cap_unit','bill_period','billing_frequency','billing_cycle_config_id','billing_cycle_calendar_id'],
'bucket_request_error_history':['error_history_id','request_id','customer_ban','attempt_number','error_code','error_message','existing_bucket_id','failed_dt'],
...
'bucket_validity_period':['validity_period_id','validity_period_name','bucket_type_group_id','validity_duration','validity_unit','recurrence_type','year','month','day_of_month','day_of_week_mask','start_time','end_time','next_day_end','status','valid_from','valid_to','priority','description','created_dt','created_by','tenant_id'],
...
'billing_cycle_config':['billing_cycle_config_id','bill_period','billing_frequency','bill_period_display','billing_frequency_display','status','tenant_id','valid_from','valid_to','created_dt','updated_dt'],
'billing_cycle_calendar':['billing_cycle_calendar_id','billing_cycle_config_id','ppdd_date','next_ppdd_date','statement_date','cutoff_date','prep_delay','prep_date','status','created_dt','updated_dt'],
'customer_bucket':['bucket_id','customer_ban','subscr_id','tenant_id','status','bucket_usage_type','bucket_validity_period','bucket_usage_group','bucket_trigger_type','bucket_trigger_id','order_id','product_offering_id','pop_id','product_spec_char_value_use_id','is_rollover_enabled','rollover_cap_value','rollover_cap_unit','recurrence_type','validity_year','validity_month','day_of_month','day_of_week_mask','timeband_start','timeband_end','next_day_end','priority','valid_start_time','valid_end_time','created_at','last_updated_at','bucket_unit','bucket_initial_value','bucket_remaining_value','bill_period','billing_frequency','billing_cycle_config_id','billing_cycle_calendar_id','ppdd_date','next_ppdd_date','statement_date','cutoff_date']}
    ...
    with get_connection() as conn:
        ...
        for r in rows:
            ...
            try:
                ...
                # BILLING_FREQUENCY must be numeric because the charging configuration uses INTEGER.
                try:
                    billing_frequency=int(str(r['billing_frequency']).strip())
                except (ValueError,TypeError):
                    mark_inbound_failed(
                        conn,
                        inbound_id,
                        'INVALID_BILLING_FREQUENCY',
                        f"BILLING_FREQUENCY={r['billing_frequency']} is not numeric"
                    )
                    conn.commit()
                    rejected+=1
                    continue
                ...
                if existing:
                    same_request=(
                        ...
                        and str(existing['bill_period']).strip().upper()
                            ==str(r['bill_period']).strip().upper()
                        and str(existing['billing_frequency'])==str(billing_frequency)
                    )
...
def pick_pending_requests(conn):
    with conn.cursor() as cur:
        cur.execute(f"""SELECT request_id,customer_ban,subscr_id,product_offering_id,pop_id,order_id,bucket_trigger_type,bill_period,billing_frequency FROM {DB_SCHEMA}.bucket_request WHERE process_status='PENDING' ORDER BY request_id FOR UPDATE SKIP LOCKED""")
        rows=cur.fetchall()
        for r in rows:
...
def validate_bill_cycle(conn,r,tenant_id):
    """
    Validate the customer-selected Bill Period and Billing Frequency received
    from PCAT and resolve the applicable postpaid billing calendar record.

    Important:
      PPDD_DATE / NEXT_PPDD_DATE are postpaid due-date milestones. They are
      stored for audit and later billing-cycle processing, but this version
      does NOT reinterpret them as CUSTOMER_BUCKET valid_start_time/end_time.
    """
    bill_period = None if r.get('bill_period') is None else str(r['bill_period']).strip()
    billing_frequency = r.get('billing_frequency')

    if not bill_period:
        raise BucketProcessingError('BILL_PERIOD_MISSING','Bill Period is mandatory from PCAT')
    if billing_frequency is None or str(billing_frequency).strip()=='':
        raise BucketProcessingError('BILLING_FREQUENCY_MISSING','Billing Frequency is mandatory from PCAT')

    try:
        billing_frequency = int(billing_frequency)
    except (TypeError,ValueError):
        raise BucketProcessingError('INVALID_BILLING_FREQUENCY',f'Billing Frequency={billing_frequency} is not numeric')

    logger.info(
        'REQUEST | %s | STEP4 BILL CYCLE validate | bill_period=%s frequency=%s tenant=%s',
        r['request_id'],bill_period,billing_frequency,tenant_id
    )
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            SELECT
                ...
                status,
                tenant_id
            FROM {DB_SCHEMA}.billing_cycle_config
            WHERE UPPER(TRIM(bill_period))=UPPER(TRIM(%s))
              AND billing_frequency=%s
              AND tenant_id=%s
              AND UPPER(TRIM(status))='ACTIVE'
              AND valid_from<=CURRENT_TIMESTAMP
              AND (valid_to IS NULL OR CURRENT_TIMESTAMP<valid_to)
            """,
            (bill_period,billing_frequency,tenant_id)
        )
        config_rows=cur.fetchall()

    if not config_rows:
        raise BucketProcessingError(
            'BILL_PERIOD_FREQUENCY_INVALID',
            f'No ACTIVE billing configuration for bill_period={bill_period} billing_frequency={billing_frequency} tenant={tenant_id}'
        )
    if len(config_rows)>1:
        raise BucketProcessingError(
            'MULTIPLE_BILL_CYCLE_CONFIG_FOUND',
            f'Multiple ACTIVE billing configurations for bill_period={bill_period} billing_frequency={billing_frequency} tenant={tenant_id}'
        )

    cfg=config_rows[0]

    # Resolve the nearest applicable billing calendar row.
    # PPDD_DATE is a due date, not bucket validity.
    with conn.cursor() as cur:
        cur.execute(
            f"""
            SELECT
                billing_cycle_calendar_id,
                billing_cycle_config_id,
                ppdd_date,
                next_ppdd_date,
                statement_date,
                ...
                prep_date,
                status
            FROM {DB_SCHEMA}.billing_cycle_calendar
            WHERE billing_cycle_config_id=%s
              AND UPPER(TRIM(status))='ACTIVE'
              AND next_ppdd_date>=CURRENT_DATE
            ORDER BY
                CASE WHEN ppdd_date>=CURRENT_DATE THEN 0 ELSE 1 END,
                ppdd_date
            LIMIT 1
            """,
            (cfg['billing_cycle_config_id'],)
        )
        cal=cur.fetchone()

    if not cal:
        raise BucketProcessingError(
            'BILL_CYCLE_CALENDAR_NOT_FOUND',
            f'No applicable billing calendar for bill_period={bill_period} billing_frequency={billing_frequency} tenant={tenant_id}'
        )

    if cal['next_ppdd_date'] <= cal['ppdd_date']:
        raise BucketProcessingError(
            'BILL_CYCLE_DATE_INVALID',
            f"NEXT_PPDD_DATE={cal['next_ppdd_date']} must be after PPDD_DATE={cal['ppdd_date']}"
        )

    result={
        'billing_cycle_config_id':cfg['billing_cycle_config_id'],
        'billing_cycle_calendar_id':cal['billing_cycle_calendar_id'],
        'bill_period':cfg['bill_period'],
        'billing_frequency':cfg['billing_frequency'],
        'bill_period_display':cfg['bill_period_display'],
        'billing_frequency_display':cfg['billing_frequency_display'],
        'ppdd_date':cal['ppdd_date'],
        'next_ppdd_date':cal['next_ppdd_date'],
        'statement_date':cal['statement_date'],
        'cutoff_date':cal['cutoff_date'],
        'prep_delay':cal['prep_delay'],
        'prep_date':cal['prep_date']
    }

    logger.info(
        'REQUEST | %s | STEP4 BILL CYCLE SUCCESS | config_id=%s calendar_id=%s bill_period=%s frequency=%s ppdd=%s next_ppdd=%s statement=%s cutoff=%s',
        r['request_id'],result['billing_cycle_config_id'],result['billing_cycle_calendar_id'],
        result['bill_period'],result['billing_frequency'],result['ppdd_date'],
        result['next_ppdd_date'],result['statement_date'],result['cutoff_date']
    )
    return result
...
def get_groups(conn,r,u):
    ...
    Resolution path (final sequence):
      1. USG_BUCKET_GRP gives candidate groups for the Usage Spec.
      2. POP_BUCKET_GRP selects the exact group + tenant for POP + Usage Spec.
      3. Bill Cycle is validated next using the resolved tenant.
      4. PO_BUCKET_GRP is validated only AFTER Bill Cycle validation.

    This intentionally keeps PO validation out of this function so the runtime
    order is: Group/Tenant -> Bill Cycle -> PO -> Bucket Definition.
    """
    logger.info(
    ...
    # 2. Resolve the exact POP -> Usage Spec -> Bucket Group + Tenant mapping.
    #    IMPORTANT: PO_BUCKET_GRP is deliberately NOT joined here.
    #    Bill Cycle must be validated after tenant resolution and before PO validation.
    with conn.cursor() as cur:
        cur.execute(
            f"""
            SELECT DISTINCT
    ...
    if not rows:
        raise BucketProcessingError(
            ...
            (
                f"No active POP bucket-group mapping for "
    ...
    if len(rows)>1:
        raise BucketProcessingError(
            ...
            (
                f"Multiple active POP bucket groups found for POP_ID={r['pop_id']} "
                f"USAGE_SPEC_ID={u}: "
    ...
    logger.info(
        'REQUEST | %s | STEP3 SUCCESS | POP=%s usage_spec=%s exact_group=%s tenant=%s',
        r['request_id'],r['pop_id'],u,
...
def validate_po_mapping(conn,r,g,tenant_id):
    logger.info('REQUEST | %s | STEP5 PO_BUCKET_GRP product=%s group=%s tenant=%s',r['request_id'],r['product_offering_id'],g,tenant_id)
    with conn.cursor() as cur:
    ...
    po_map=rows[0]
    logger.info('REQUEST | %s | STEP5 SUCCESS | char_id=%s value=%s',r['request_id'],po_map['product_characteristic_value_id'],po_map['product_characteristic_value'])
    return po_map

def get_bucket_definition(conn,r,g,tenant_id):
    logger.info('REQUEST | %s | STEP6 BUCKET_DEFINITION group=%s tenant=%s',r['request_id'],g,tenant_id)
    with conn.cursor() as cur:
        cur.execute(
            f"""
            SELECT
    ...
    b=rows[0]
    logger.info(
        'REQUEST | %s | STEP6 SUCCESS | type=%s uom_id=%s uom=%s dimension=%s initial=%s validity_id=%s',
...
def get_validity(conn,r,vid,g,tenant_id):
    """
    Resolve complete bucket validity from the NEW public.bucket_validity_period.

    This table is now the single source for:
      - customer bucket lifetime:
            validity_duration + validity_unit
      - recurring eligibility:
            recurrence_type / day/month/week / start_time / end_time
      - cross-midnight handling:
            next_day_end
      - campaign/configuration lifetime:
            valid_from / valid_to
      - deduction preference:
            priority

    bucket_validity_period_old is backup/reference only and is never queried.
    """
    logger.info(
        'REQUEST | %s | STEP7 BUCKET_VALIDITY_PERIOD id=%s group=%s tenant=%s',
        r['request_id'], vid, g, tenant_id
    )
    ...
    with conn.cursor() as cur:
        cur.execute(
            f"""
            SELECT
                bvp.validity_period_id,
                bvp.validity_period_name,
                bvp.bucket_type_group_id,
                bvp.validity_duration,
                bvp.validity_unit,
                vu.validity_unit_name,
                bvp.recurrence_type,
                bvp.year,
                bvp.month,
                bvp.day_of_month,
                bvp.day_of_week_mask,
                bvp.start_time,
                bvp.end_time,
                COALESCE(bvp.next_day_end,FALSE) AS next_day_end,
                bvp.status,
                bvp.valid_from,
                bvp.valid_to,
                COALESCE(bvp.priority,0) AS priority,
                bvp.description,
                bvp.tenant_id
            FROM {DB_SCHEMA}.bucket_validity_period bvp
            JOIN {DB_SCHEMA}.validity_unit vu
              ON vu.validity_unit_id=bvp.validity_unit
            WHERE bvp.validity_period_id=%s
              AND bvp.bucket_type_group_id=%s
              AND bvp.tenant_id=%s
              AND bvp.status=1
              AND CURRENT_TIMESTAMP>=bvp.valid_from
              AND (bvp.valid_to IS NULL OR CURRENT_TIMESTAMP<bvp.valid_to)
            """,
            (vid, str(g), tenant_id)
        )
        rows=cur.fetchall()
    ...
    if not rows:
        raise BucketProcessingError(
            'VALIDITY_PERIOD_NOT_FOUND',
            (
                f'Validity period {vid} missing/inactive/outside dates '
                f'or not mapped to group={g} tenant={tenant_id}'
            )
        )

    if len(rows)>1:
        raise BucketProcessingError(
            'MULTIPLE_VALIDITY_PERIOD_FOUND',
            (
                f'Multiple active validity rows found for validity_period_id={vid} '
                f'group={g} tenant={tenant_id}'
            )
        )

    v=rows[0]

    if v['validity_duration'] is None or int(v['validity_duration']) <= 0:
        raise BucketProcessingError(
            'INVALID_VALIDITY_DURATION',
            f'validity_duration must be greater than zero for validity_period_id={vid}'
        )

    recurrence=(v['recurrence_type'] or 'DAILY').strip().upper()
    allowed={'DAILY','WEEKLY','MONTHLY','YEARLY','ONCE'}

    if recurrence not in allowed:
        raise BucketProcessingError(
            'INVALID_RECURRENCE_TYPE',
            f"Unsupported recurrence_type={v['recurrence_type']} for validity_period_id={vid}"
        )

    # Time-band validation. NULL/NULL means no time restriction.
    if (v['start_time'] is None) != (v['end_time'] is None):
        raise BucketProcessingError(
            'INVALID_TIMEBAND_CONFIGURATION',
            (
                f'Both start_time and end_time must be populated together '
                f'for validity_period_id={vid}'
            )
        )

    if v['next_day_end'] and (v['start_time'] is None or v['end_time'] is None):
        raise BucketProcessingError(
            'INVALID_TIMEBAND_CONFIGURATION',
            f'next_day_end=TRUE requires start_time and end_time for validity_period_id={vid}'
        )

    if (
        v['start_time'] is not None
        and v['end_time'] is not None
        and v['start_time'] > v['end_time']
        and not v['next_day_end']
    ):
        raise BucketProcessingError(
            'INVALID_TIMEBAND_CONFIGURATION',
            (
                f'Cross-midnight rule {v["start_time"]}-{v["end_time"]} '
                f'requires next_day_end=TRUE for validity_period_id={vid}'
            )
        )

    if recurrence == 'WEEKLY':
        mask=v['day_of_week_mask']
        if mask is None or int(mask) < 1 or int(mask) > 127:
            raise BucketProcessingError(
                'INVALID_WEEKLY_RECURRENCE',
                f'WEEKLY recurrence requires day_of_week_mask 1-127 for validity_period_id={vid}'
            )

    if recurrence == 'MONTHLY':
        # For postpaid monthly buckets, day_of_month is optional.
        # The bucket lifetime is controlled by validity_duration + validity_unit
        # (for example, 1 MONTH), and the next cycle starts from the previous
        # bucket's valid_end_time. Validate day_of_month only when configured.
        dom=v['day_of_month']
        if dom is not None:
            try:
                dom=int(dom)
                ...
                raise BucketProcessingError(
                    'INVALID_MONTHLY_RECURRENCE',
                    f'Invalid day_of_month={v["day_of_month"]} for validity_period_id={vid}; expected 1-31 or NULL'
                )

            if dom < 1 or dom > 31:
                raise BucketProcessingError(
                    'INVALID_MONTHLY_RECURRENCE',
                    f'Invalid day_of_month={dom} for validity_period_id={vid}; expected 1-31 or NULL'
                )

            # Keep the normalized integer value in the resolved validity snapshot.
            v['day_of_month']=dom
    ...
    if recurrence == 'YEARLY':
        ...
        dom=v['day_of_month']
        if (
            month is None or int(month) < 1 or int(month) > 12
            or dom is None or int(dom) < 1 or int(dom) > 31
        ):
            raise BucketProcessingError(
                'INVALID_YEARLY_RECURRENCE',
                f'YEARLY recurrence requires valid month/day_of_month for validity_period_id={vid}'
            )

    # Persist the normalized value in the resolved validity snapshot so
    # CUSTOMER_BUCKET always stores one canonical recurrence value.
    v['recurrence_type']=recurrence

    logger.info(
        'REQUEST | %s | STEP7 SUCCESS | id=%s name=%s lifetime=%s %s '
        'recurrence=%s time=%s-%s next_day_end=%s priority=%s',
        r['request_id'],
        v['validity_period_id'],
        v['validity_period_name'],
        v['validity_duration'],
        v['validity_unit_name'],
        recurrence,
        v['start_time'],
        v['end_time'],
        v['next_day_end'],
...
def validate_trigger(conn,r,g,tenant_id):
    logger.info('REQUEST | %s | STEP8 trigger=%s group=%s tenant=%s',r['request_id'],r['bucket_trigger_type'],g,tenant_id)
    with conn.cursor() as cur:
        cur.execute(
            f"""
    ...
    trigger=rows[0]
    logger.info(
        'REQUEST | %s | STEP8 SUCCESS | trigger_id=%s trigger_type=%s',
...
def get_rollover_config(conn,r,g,tenant_id,bucket_uom_id):
    logger.info(
        'REQUEST | %s | STEP9 BUCKET_ROLLOVER_CONFIG product=%s group=%s tenant=%s',
...
def find_existing(conn,r,g):
    """
    Reject another currently usable bucket for the same subscriber entitlement.

    Runtime identity used here:
      BAN + SUBSCR_ID + PRODUCT_OFFERING_ID + BUCKET_GROUP

    USAGE_SPEC_ID is intentionally not stored in CUSTOMER_BUCKET anymore.
    It is still used during configuration resolution and retained in
    BUCKET_REQUEST as an audit snapshot.

    CLOSED/EXHAUSTED/expired buckets do not block a new creation.
    """
    logger.info(
        'REQUEST | %s | PRECHECK BUCKET checking BAN + SUBSCR + PRODUCT + BUCKET_GROUP',
        r['request_id']
    )
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            WHERE customer_ban::text = %s
              AND subscr_id::text = %s
              AND product_offering_id::text = %s
              AND bucket_usage_group::text = %s
              AND UPPER(TRIM(status)) IN ('ACTIVE','DEPLETING')
              AND (
    ...
    if x:
        ...
        logger.info(
            'REQUEST | %s | PRECHECK BUCKET SUCCESS | no existing usable bucket',
            r['request_id']
        )
...
def get_db_current_timestamp(conn):
    """Use PostgreSQL time as the authoritative bucket activation timestamp."""
    with conn.cursor() as cur:
        cur.execute('SELECT CURRENT_TIMESTAMP AS current_ts')
        row=cur.fetchone()
...
def update_bucket_request_resolution(conn,r,usage_spec_id,g,po_map,b,trigger,rollover,bill_cycle):
    """
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            SET usage_spec_id=%s,
                ...
                bill_period=%s,
                billing_frequency=%s,
                billing_cycle_config_id=%s,
                billing_cycle_calendar_id=%s,
                updated_dt=CURRENT_TIMESTAMP
            WHERE request_id=%s
            """,
            (
                ...
                bill_cycle['bill_period'],
                bill_cycle['billing_frequency'],
                bill_cycle['billing_cycle_config_id'],
                bill_cycle['billing_cycle_calendar_id'],
                r['request_id']
            )
        )
...
def insert_bucket(conn,r,g,tenant_id,b,v,po_map,trigger,rollover,bill_cycle):
    """
    Create the runtime CUSTOMER_BUCKET snapshot.

    The bucket keeps the resolved operational validity values so the deduction
    engine can decide eligibility without re-resolving configuration tables.
    """
    logger.info('REQUEST | %s | STEP9 prepare CUSTOMER_BUCKET',r['request_id'])
    bid=generate_bucket_id(conn)
    start=get_db_current_timestamp(conn)
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            (
                ...
                bill_cycle['billing_cycle_config_id'],
                bill_cycle['billing_cycle_calendar_id'],
                bill_cycle['ppdd_date'],
                bill_cycle['next_ppdd_date'],
                bill_cycle['statement_date'],
                bill_cycle['cutoff_date']
            )
        )

    logger.info(
        'REQUEST | %s | STEP9 SUCCESS | bucket_id=%s status=%s group=%s recurrence=%s priority=%s valid=%s..%s initial=%s remaining=%s',
        r['request_id'],bid,CUSTOMER_BUCKET_ACTIVE_STATUS,g,
        v['recurrence_type'],v['priority'],start,end,initial,initial
    )
    return bid
...
def mark_failed(conn,rid,code,msg,existing_bucket_id=None):
    """Mark the latest request state as FAILED and preserve this attempt in error history."""
    safe_message=(msg or '')[:1000]

    with conn.cursor() as cur:
        # Keep BUCKET_REQUEST as the latest/current state.
        # RETURNING gives the attempt number that was incremented when the
        # request moved from PENDING -> PROCESSING.
        cur.execute(
            f"""
            UPDATE {DB_SCHEMA}.bucket_request
            SET process_status='FAILED',
                ...
                error_message=%s
            WHERE request_id=%s
            RETURNING process_attempt_count,customer_ban
            """,
            (
                existing_bucket_id,
                code,
        ...
        if not updated:
            raise RuntimeError(
                f'Unable to mark request_id={rid} as FAILED: request not found'
            )

        attempt_number=updated['process_attempt_count']
        customer_ban=updated['customer_ban']

        # Preserve every failed attempt. On a later successful retry,
        # BUCKET_REQUEST.error_code/error_message can be cleared safely because
        # the previous failures remain available here.
        cur.execute(
            f"""
            INSERT INTO {DB_SCHEMA}.bucket_request_error_history
            (
                request_id,
                customer_ban,
                attempt_number,
    ...
    logger.error(
        'REQUEST | %s | FINAL FAILED | attempt=%s | code=%s | existing_bucket_id=%s | message=%s',
        rid,
        attempt_number,
        code,
        existing_bucket_id,
    ...
    logger.info(
        'REQUEST | %s | ERROR HISTORY SAVED | attempt=%s | code=%s',
        rid,
        attempt_number,
        code
    )
...
def process_request(conn,r):
    ...
    for row in groups:
        ...
        # STEP 4: Mandatory Bill Cycle validation from PCAT.
        # At this point Bucket Group + Tenant are known, while PO/Definition
        # validation has NOT yet been performed.
        bill_cycle=validate_bill_cycle(conn,r,tenant_id)
        ...
        # STEP 5 onward: continue entitlement configuration only after Bill Cycle succeeds.
        po_map=validate_po_mapping(conn,r,g,tenant_id)
        b=get_bucket_definition(conn,r,g,tenant_id)
        v=get_validity(conn,r,b['validity_period_id'],g,tenant_id)
        ...
        update_bucket_request_resolution(
            conn,r,usage_spec_id,g,po_map,b,trigger,rollover,bill_cycle
        )

        bids.append(
            insert_bucket(conn,r,g,tenant_id,b,v,po_map,trigger,rollover,bill_cycle)
        )
...
def main():
    ...
    logger.info(
        ...
        'inbound_rejected=%s | total=%s | success=%s | failed=%s | runtime_errors=%s',
