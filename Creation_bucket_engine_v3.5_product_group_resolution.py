...
def get_groups(conn,r,u):
    """
    Resolve bucket group(s) for the request.

    A single USAGE_SPEC_ID can legitimately map to multiple bucket groups.
    Example:
        23103 -> 103
        23103 -> 500

    Therefore we must not process every group returned by USG_BUCKET_GRP.
    We first get all candidate groups for the usage spec, then keep only the
    group(s) that are also mapped to the incoming PRODUCT_OFFERING_ID in
    PO_BUCKET_GRP for the same tenant.
    """
    logger.info(
        'REQUEST | %s | STEP3 USG_BUCKET_GRP usage_spec=%s',
        r['request_id'],u
    )

    with conn.cursor() as cur:
        # 1. Get all groups configured for the Usage Spec.
        cur.execute(
            f"""
    ...
    logger.info(
        'REQUEST | %s | STEP3 CANDIDATE GROUPS | groups=%s',
        r['request_id'],
        [(x['bucket_type_group_id'],x['tenant_id']) for x in candidate_rows]
    )

    # 2. Filter candidate groups using Product Offering mapping.
    with conn.cursor() as cur:
        cur.execute(
            f"""
            SELECT DISTINCT
                ubg.bucket_type_group_id,
                ubg.tenant_id
            FROM {DB_SCHEMA}.usg_bucket_grp ubg
            JOIN {DB_SCHEMA}.po_bucket_grp pbg
              ON pbg.bucket_type_group_id = ubg.bucket_type_group_id
             AND pbg.tenant_id = ubg.tenant_id
            WHERE ubg.usage_spec_id::text = %s
              AND pbg.product_offering_id::text = %s
            ORDER BY ubg.bucket_type_group_id
            """,
            (str(u),r['product_offering_id'])
        )
        rows=cur.fetchall()

    if not rows:
        raise BucketProcessingError(
            'PO_BUCKET_MAPPING_NOT_FOUND',
            (
                f"No bucket group mapping found for "
                f"USAGE_SPEC_ID={u} PRODUCT_OFFERING_ID={r['product_offering_id']}"
            )
        )

    logger.info(
        'REQUEST | %s | STEP3 SUCCESS | product=%s selected_groups=%s',
        r['request_id'],
...
def main():
    print("Bucket Engine V3.5 started")

    logger.info('='*100)
    logger.info('Bucket Engine V3.5 starting')
    ...
    try:
        ...
        print("Error Count      : 1")
        print("")
        print("Bucket Engine V3.5 completed with errors")
    ...
    try:
        ...
        logger.info('Bucket Engine V3.5 completed')
        ...
        if inbound_received==0 and total==0:
            print("Bucket Engine V3.5 completed - No pending requests")
        elif overall_errors==0:
            print("Bucket Engine V3.5 completed successfully")
